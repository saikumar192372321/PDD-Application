        ...
        ScrollView(showsIndicators: false) {
            VStack(spacing: 32) {
                ...
                VStack(spacing: 24) {
                    ...
                    VanguardInput(label: "OFFER TITLE", text: $title, icon: "tag.fill")
                    VanguardInput(label: "FIELD DESCRIPTION", text: $description, icon: "text.alignleft")
                    
                    SectionHeader(title: "Compliance Conditions")
                    VanguardInput(label: "MINIMUM ORDER THRESHOLD (₹)", text: $minOrderValue, icon: "cart.fill.badge.plus", keyboard: .numberPad)
