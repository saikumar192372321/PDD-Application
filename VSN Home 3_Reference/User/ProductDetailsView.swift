            ...
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    VStack(alignment: .leading, spacing: 24) {
                        ...
                        // Brand + Stock status
                        HStack {
                            Text(product.details?.brand!.uppercased() ?? "")
                        ...
                        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                            ...
                            InfoTile(label: "Category", value: (product.details?.category.rawValue)!, icon: "tag.fill", color: AppColors.accent)
                            InfoTile(label: "Net Quantity", value: product.details?.netQuantity ?? "", icon: "scalemass.fill", color: AppColors.secondary)
