...
if ($result && $result->num_rows > 0) {
    ...
    $isValid = ($stored === $password) // plain-text match (legacy)
            || password_verify($password, $stored); // bcrypt match

    if ($isValid) {
        // Auto-upgrade plain-text password to bcrypt on successful login
        if ($stored === $password) {
            $hash = password_hash($password, PASSWORD_BCRYPT);
            $conn->query("UPDATE admins SET password = '$hash' WHERE email = '$email'");
        }
