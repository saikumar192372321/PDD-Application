...
struct AdminTabView: View {
    ...
    @State private var adminUPI: String = ""
    ...
    var body: some View {
        ...
        .safeAreaInset(edge: .leading) { Color.clear.frame(width: 16) }
        .safeAreaInset(edge: .trailing) { Color.clear.frame(width: 16) }
        ...
        .onAppear {
            ...
            // Load admin credentials from Keychain
            if let email = try? KeychainManager.retrieve(key: "adminEmail", type: String.self) {
                adminEmail = email
            }
            if let upi = try? KeychainManager.retrieve(key: "adminUPI", type: String.self) {
                adminUPI = upi
            }
        }
    }
    ...
    private var tabContent: some View {
        Group {
            ...
            case .insights:
                NavigationStack {
                    AdminDashboardView(productStore: productStore, notificationStore: notificationStore, selectedTab: $selectedTab, selectedLanguage: $selectedLanguage)
                }
            case .inventory:
                NavigationStack {
                    ProductListView(productStore: productStore, notificationStore: notificationStore)
                }
            case .orders:
                NavigationStack {
            ...
            case .profile:
                NavigationStack {
                    AdminProfileView(onLogout: onLogout, adminEmail: adminEmail, adminUPI: adminUPI)
                }
            }
        }
        .toolbar(.hidden, for: .tabBar)
        .padding(.bottom, tabBarState.isHidden ? 0 : 90)
    }
    ...
    private var liquidTabBar: some View {
        HStack(spacing: 0) {
            ForEach(AdminTab.allCases, id: \.self) { tab in
                AdminTabButton(
                    tab: tab,
                    icon: adminTabIcon(for: tab),
                    label: adminTabLabel(for: tab),
    ...
    private func adminTabIcon(for tab: AdminTab) -> String {
        switch tab {
        case .insights: return "chart.bar.xaxis"
        case .inventory: return "archivebox.fill"
        case .orders: return "shippingbox.fill"
        case .partners: return "person.2.fill"
        case .broadcast: return "bell.badge.fill"
        case .profile: return "person.fill"
        }
    }
    
    private func adminTabLabel(for tab: AdminTab) -> String {
        switch tab {
        case .insights: return AppText.get("tab_insights", lang: selectedLanguage)
        case .inventory: return AppText.get("tab_stock", lang: selectedLanguage)
        case .orders: return AppText.get("tab_orders", lang: selectedLanguage)
        case .partners: return AppText.get("tab_partners", lang: selectedLanguage)
        case .broadcast: return AppText.get("tab_alerts", lang: selectedLanguage)
        ...
        Button(action: {
            ...
            VStack(spacing: 4) {
                ZStack {
                    if isSelected {
                        Capsule()
                            ...
                            .matchedGeometryEffect(id: "adminHighlight", in: namespace)
                    }
                    
                    Image(systemName: icon)
                        .font(.system(size: 18, weight: isSelected ? .bold : .medium))
                        .foregroundColor(isSelected ? AppColors.primary : AppColors.textSecondary.opacity(0.6))
                ...
                Text(label)
                    ...
                    .foregroundColor(isSelected ? AppColors.primary : AppColors.textSecondary.opacity(0.7))
                    .frame(height: 10)
            }
...
struct AdminProfileView: View {
    ...
    @State private var deliveryCharge = ""
        ...
        ZStack {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 28) {
                    ...
                    // System Info
                    VStack(spacing: 1) {
                        AdminInfoRow(title: "Admin ID", value: adminEmail.isEmpty ? "admin@vsn.com" : adminEmail, icon: "person.fill", color: AppColors.primary)
                        AdminInfoRow(title: "Role", value: "Super Admin", icon: "shield.checkered", color: .purple)
                        if !adminUPI.isEmpty {
                            AdminInfoRow(title: "Personal UPI", value: adminUPI, icon: "indianrupeesign.circle.fill", color: .orange)
                        }
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            .overlay(RoundedRectangle(cornerRadius: 12).stroke(AppColors.primary.opacity(0.15), lineWidth: 1))
            ...
            do {
                ...
                if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                    await MainActor.run {
                        ...
                        self.deliveryCharge = "\(json["delivery_charge"] ?? "0")"
                        self.deliveryNote = "\(json["delivery_note"] ?? "")"
                        self.adminMasterKey = "" // Don't expose master key from backend
                    }
                }
            } catch {
    ...
    private func updateLogisticsDetails() {
        ...
        Task {
            do {
                ...
                if !adminMasterKey.isEmpty {
                    bodyParams["admin_master_key"] = adminMasterKey
                }
