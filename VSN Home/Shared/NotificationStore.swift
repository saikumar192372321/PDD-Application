...
class NotificationStore: ObservableObject {
    ...
    func clearAll(for email: String) {
        ...
        guard let url = URL(string: baseURL + "clear_notifications.php") else { return }
        ...
        Task {
            do {
                ...
                if response.status != "success" {
                    print("Clear all failed on server: \(response.message ?? "unknown")")
                    // Optionally re-fetch to restore state if delete failed
                    await fetchNotifications(userEmail: email)
                }
            } catch {
                print("Clear all failed: \(error)")
                await fetchNotifications(userEmail: email)
            }
        }
    }
