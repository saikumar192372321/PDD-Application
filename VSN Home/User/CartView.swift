...
struct CartView: View {
    ...
    @State private var minOrderValue: Double = 1000.0 // Fetched from admin settings
    @State private var deliveryNote: String = ""
    @State private var isFetchingLogistics = true
    @State private var applyCoins = false
    @State private var coinConversionRate: Double = 0.1 // Default fallback
    ...
    var coinDiscount: Double {
        guard applyCoins else { return 0 }
        return Double(userCoins) * coinConversionRate
    }
    ...
    var finalPrice: Double {
        ...
        return max(0, totalPrice - discountAmount - coinDiscount + charge)
    }
    ...
    var body: some View {
                ...
                if cartItems.isEmpty {
                    ...
                    ScrollView(showsIndicators: false) {
                        VStack(spacing: 24) {
                            ...
                            VStack(alignment: .leading, spacing: 18) {
                                HStack {
                                    VStack(alignment: .leading, spacing: 2) {
                                        ...
                                        Text("\(cartItems.count) SKUs in Cart")
                                ...
                                VStack(alignment: .leading, spacing: 12) {
                                    HStack {
                                        VStack(alignment: .leading, spacing: 4) {
                                            HStack(spacing: 6) {
                                                ...
                                                Text(AppText.get("use_coins", lang: selectedLanguage))
                                                    .font(.system(size: 15, weight: .bold))
                                            }
                                            Text("\(String(format: AppText.get("coins_available", lang: selectedLanguage), userCoins)) (\(AppText.get("worth", lang: selectedLanguage)) ₹\(Int(coinDiscount == 0 && !applyCoins ? Double(userCoins) * coinConversionRate : coinDiscount)))")
                                                .font(.system(size: 11))
                                                .foregroundColor(AppColors.textSecondary)
                                        }
                                        Spacer()
                                        Toggle("", isOn: $applyCoins)
                                            .tint(.orange)
                                            .labelsHidden()
                                    }
                                    .padding(16)
                                    ...
                                    .overlay(
                                        RoundedRectangle(cornerRadius: 16)
                                            .stroke(applyCoins ? Color.orange.opacity(0.3) : Color.clear, lineWidth: 1)
                                    )
                                }
                                .padding(.horizontal, 20)
                            }
                            ...
                            // 6. Final Summary
                            SummaryCard(totalPrice: totalPrice, discountAmount: discountAmount, coinDiscount: coinDiscount, deliveryCharge: deliveryCharge, deliveryNote: deliveryNote, freeThreshold: freeThreshold, finalPrice: finalPrice, totalSavings: totalSavings, earnedCoins: earnedCoins, selectedOffer: selectedOffer, cartItems: cartItems)
                                .padding(.horizontal, 20)
        ...
        .navigationTitle(AppText.get("tab_cart", lang: selectedLanguage))
        ...
        .safeAreaInset(edge: .bottom) {
            if !cartItems.isEmpty {
                CheckoutFooter(
                    ...
                    isLoading: isLoading,
                    selectedLanguage: selectedLanguage, // New param
                    onCheckout: {
    ...
    private func fetchLogisticsSettings() {
        guard let url = URL(string: APIConfig.getSupport) else { return }
        Task {
            do {
                ...
                if let json = try? JSONSerialization.jsonObject(with: data) as? [String: String] {
                    await MainActor.run {
                        ...
                        self.minOrderValue = Double(json["min_order_value"] ?? "1000") ?? 1000.0
                        self.coinConversionRate = Double(json["coin_conversion_rate"] ?? "0.1") ?? 0.1
    ...
    private func placeOrder() {
        ...
        let newOrder = Order(
            ...
            coinsUsed: applyCoins ? userCoins : 0,
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    if response.status == "success" {
                        ...
                        selectedOffer = nil
                        if applyCoins {
                            userCoins -= Int(userCoins) // Deduct all if applied
                        }
...
struct EmptyCartPlaceholder: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            Text("You haven't added any products to your wholesale choice.")
                .font(.system(size: 14))
                .foregroundColor(AppColors.textSecondary)
    ...
    var body: some View {
        VStack(spacing: 18) {
            VStack(spacing: 12) {
                ...
                if coinDiscount > 0 {
                    SummaryLineView(label: "Coin Discount", value: "-₹\(Int(coinDiscount))", color: .orange)
                }
...
struct CheckoutFooter: View {
    ...
    /// Human-readable reason why checkout is blocked (nil when eligible)
    var blockReason: String? {
        if currentDistance > allowedRadius { return "Outside delivery zone" }
        if finalPrice < minOrderValue { return "Min order ₹\(Int(minOrderValue)) required" }
        if requiresGST && businessGSTName.isEmpty { return "Enter business name for GST" }
        if requiresGST && gstNumber.count != 15 { return "GSTIN must be 15 digits" }
        return nil
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if let reason = blockReason {
                HStack(spacing: 8) {
                    Image(systemName: finalPrice < minOrderValue ? "cart.badge.plus" : "exclamationmark.triangle.fill")
            ...
            HStack(spacing: 16) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(isEligible ? "FINAL TOTAL" : "ORDER TOTAL")
                        .font(.system(size: 10, weight: .black))
                        .foregroundColor(!isEligible ? .orange : AppColors.textSecondary)
                        .tracking(1)

                    Text("₹\(Int(finalPrice))")
                        .font(.system(size: 24, weight: .black))
                        .foregroundColor(!isEligible ? .orange : AppColors.primary)
                }
                ...
                Button(action: onCheckout) {
                    HStack {
                        ...
                        } else if finalPrice < minOrderValue {
                            VStack(spacing: 2) {
                                Text("MIN ₹\(Int(minOrderValue))")
                                    .font(.system(size: 11, weight: .black))
                                Text("REQUIRED")
                                    .font(.system(size: 9, weight: .heavy))
                            }
                        } else {
                            Text(isEligible ? AppText.get("order_placed", lang: selectedLanguage).uppercased() : AppText.get("checkout", lang: selectedLanguage).uppercased())
                                .font(.system(size: 14, weight: .black))
                            Image(systemName: isEligible ? "chevron.right" : "lock.fill")
                                .font(.system(size: 12, weight: .black))
                        }
                    }
                    .padding(.horizontal, 24)
                    ...
                    .foregroundColor(isEligible ? .white : (finalPrice < minOrderValue ? .orange : Color.gray))
                    .cornerRadius(16)
        ...
        .animation(.spring(response: 0.35, dampingFraction: 0.85), value: blockReason)
    }
}
