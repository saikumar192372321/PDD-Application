    ...
    var body: some View {
        ZStack {
            ...
            ScrollView(showsIndicators: false) {
                ...
                // 1. Profile Hero Header — flush to top, no blank gap
                ZStack(alignment: .bottom) {
                    // Gradient banner — extends behind status bar
                    LinearGradient(
                        colors: [AppColors.primary.opacity(0.22), AppColors.primary.opacity(0.05)],
                    ...
                    VStack(spacing: 10) {
                        Image("AppLogo")
                            ...
                            .shadow(color: AppColors.primary.opacity(0.2), radius: 14, x: 0, y: 6)
                            .overlay(
                                RoundedRectangle(cornerRadius: 22)
                                    .stroke(Color.white.opacity(0.6), lineWidth: 2)
                            )
                            .offset(y: 45) // Peek out below the banner
                    }
                }
                
                // Name & badges — positioned after the avatar overlap
                VStack(spacing: 6) {
                    Text(SessionManager.shared.userName.isEmpty
                         ? (userEmail.components(separatedBy: "@").first?.capitalized ?? "Partner")
                         : SessionManager.shared.userName)
                        .font(.system(size: 22, weight: .bold))
                        .foregroundColor(AppColors.textPrimary)
                    
                    if !SessionManager.shared.userPhone.isEmpty {
                        Text(SessionManager.shared.userPhone)
                ...
                .padding(.top, 56) // Space for the overlapping avatar
                .padding(.bottom, 20)
                ...
                HStack(spacing: 16) {
                    MetricInfoCard(
                        title: AppText.get("loyalty_coins", lang: selectedLanguage),
                        value: "\(coins)",
                        subtitle: "≈ ₹\(Int(Double(coins) * 0.1))", // Hardcoded 0.1 for now or we could fetch it
                        icon: "pentagon.fill",
                        color: .orange
                    )
                    
                    MetricInfoCard(
                        title: AppText.get("my_orders", lang: selectedLanguage),
                        value: "\(orders.count)",
                ...
                // 3. Menu Options
                VStack(spacing: 1) {
                    NavigationLink(destination: MyOrdersView(orders: orders, selectedLanguage: selectedLanguage, showShareSheet: $showShareSheet, pdfURL: $pdfURL, isGenerating: $isGenerating)) {
                        MenuRow(title: AppText.get("my_orders", lang: selectedLanguage), icon: "list.clipboard.fill", color: .blue)
                    }
                    
                    NavigationLink(destination: MyAddressView(userAddress: $userAddress, latitude: $userLatitude, longitude: $userLongitude)) {
                        MenuRow(title: AppText.get("my_addresses", lang: selectedLanguage), icon: "map.fill", color: .green)
                    }
                    
                    NavigationLink(destination: AccountSecurityView(userEmail: $userEmail)) {
                        MenuRow(title: AppText.get("settings", lang: selectedLanguage), icon: "shield.lefthalf.filled", color: .purple)
                    }
                    
                    NavigationLink(destination: HelpDeskView()) {
                        MenuRow(title: AppText.get("support", lang: selectedLanguage), icon: "headphones", color: .orange)
                    }
                    
                    // Only show Admin Portal access to verified admins
                ...
                VStack(spacing: 12) {
                    Button(action: { showLogoutConfirmation = true }) {
                        HStack {
                            ...
                            Text(AppText.get("logout", lang: selectedLanguage))
                ...
                if orders.isEmpty {
                    ...
                    ForEach(orders) { order in
                        NavigationLink(destination: OrderTrackingView(order: order, selectedLanguage: selectedLanguage)) {
                            OrderSnippetCard(order: order, selectedLanguage: selectedLanguage, showShareSheet: $showShareSheet, pdfURL: $pdfURL, isGenerating: $isGenerating)
                        }
                    }
                }
            }
    ...
    var body: some View {
        HStack(spacing: 16) {
            ...
            Button(action: {
                ...
                if isGenerating && pdfURL == nil { // Simple check if THIS one is generating
...
struct AccountSecurityView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                ...
                Text("Secure your wholesale account with a strong password.")
                    .font(.system(size: 14))
                    .foregroundColor(AppColors.textSecondary)
                ...
                .frame(maxWidth: .infinity)
                .padding()
                .background(newPassword.isEmpty || confirmPassword.isEmpty ? AppColors.textSecondary.opacity(0.3) : AppColors.primary)
                .foregroundColor(.white)
                .cornerRadius(12)
        ...
        .alert(isSuccess ? "Success" : "Error", isPresented: $showMessage) {
    ...
    private func updatePassword() {
        ...
        isLoading = true
        let baseURL = APIConfig.baseURL
        guard let url = URL(string: baseURL + "reset_password.php") else {
            isLoading = false
            return
        }
        
        let payload = ["email": userEmail, "password": newPassword]
